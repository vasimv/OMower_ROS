...
int main(int argc, char **argv) {
  ...
  ros::init(argc, argv, "omower_shutdown");
  ros::NodeHandle nh;
  ...
  if ((s2 < 801) || (s2 > 86400))
    s2 = 801;
  fprintf(stderr, "SHUTDOWN at %d for %d secs\n", s1, s2);
  // XXXX
  snprintf(buffer, sizeof(buffer), "stty -F /dev/ttyS1 speed 38400 pass8; echo %d > /dev/ttyS1; echo > /dev/ttyS1; sleep 10", s2);
  system(buffer);
  snprintf(buffer, sizeof(buffer), "stty -F /dev/ttyS1 speed 38400 pass8; echo %d > /dev/ttyS1; echo > /dev/ttyS1; sleep 10", s2);
  system(buffer);
  sleep(10);
  ...
  ros::spinOnce();
  fprintf(stderr, "sent SHUTDOWN (%d)\n", res);
  sleep(2);
}
